...
# Validate the presence of PRIVATE_KEY
if [ -z "$PRIVATE_KEY" ]; then
    echo "Missing PRIVATE_KEY. Exiting."
    exit 1
fi

# List of non-zkSync chains
...
# Target chain name passed as an argument
TARGET_CHAIN=$1
...
for CHAIN in "${CHAIN_IDS[@]}"; do
    ...
    echo "🚀 Deploying to chain: $NETWORK_NAME (Chain ID: $CHAIN_ID)"

    if [ -z "$RPC_URL" ] || [ -z "$STARGATE_ENDPOINT" ] || [ -z "$ETHERSCAN_API_KEY" ]; then
        echo "⚠️  Missing required environment variables for $NETWORK_NAME. Skipping..."
        continue
    fi
    ...
    DEPLOYER_BALANCE_WEI=$(cast balance $(cast wallet address --private-key "$PRIVATE_KEY") --rpc-url "$RPC_URL")
    DEPLOYER_BALANCE=$(echo "scale=18; $DEPLOYER_BALANCE_WEI / 10^18" | bc)
    echo "💰 Deployer Balance: $DEPLOYER_BALANCE ETH"

    # # Ensure deployer has at least 0.01 ETH
    # if (( $(echo "$DEPLOYER_BALANCE < 0.01" | bc -l) )); then
    ...
    # Check if the deployment encountered the "null response" error
    DEPLOYMENT_STATUS=$?
    if [[ $DEPLOYMENT_STATUS -ne 0 ]]; then
        echo "⚠️ Deployment encountered an error. Retrying with --resume..."
        forge script "$SCRIPT_CONTRACT_NAME" \
            --rpc-url "$RPC_URL" \
            --chain-id "$CHAIN_ID" \
            --etherscan-api-key "$ETHERSCAN_API_KEY" \
